pipeline {
    ...
    stages {
        ...
        stage('Packaging/Pushing imagae') {
            ...
            steps {
                withDockerRegistry(credentialsId: 'dockerhub', url: 'https://index.docker.io/v1/') {
                    sh 'docker build -t longnh2002/springboot .'
                    sh 'docker push longnh2002/springboot'
                }
            }
        }
        ...
        stage('Deploy Spring Boot to DEV') {
            steps {
                echo 'Deploying and cleaning'
                sh 'docker image pull longnh2002/springboot'
                sh 'docker container stop longnh2002-springboot || echo "this container does not exist" '
                sh 'docker network create dev || echo "this network exists"'
                sh 'echo y | docker container prune '

                sh 'docker container run -d --rm --name longnh2002-springboot -p 8082:8080 --network dev longnh2002/springboot'
            }
        }
